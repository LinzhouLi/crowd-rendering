...
vec4 computeTextureColor0() { // 贴图颜色
...
vec4 computeTextureColor() { // 贴图颜色
    ...
    if (u > 0.4985) u = 0.4985; // 去除中缝
    u = u * 2.;
    vec2 textureIndex = getTextureIndex(u, v);
    u = (u + textureIndex[0]) / textureCount;
    vec4 color = texture( textureData, vec2(u, v) );
    if (textureIndex[1]==0.){//位于面部区域，进行融合
        ...
        color=color0//(color0+w1*color1+w2*color2+w3*color3)/(1.+w0+w1+w2+w3);
    }
    return color ;   
